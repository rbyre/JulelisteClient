...
const EditerBrukere = () => {
  ...
  const addUser = async () => {
    await Axios.post("https://runar-juleliste.herokuapp.com/addUser", {
      navn: navn,
      epost: epost,
  ...
  const getUsers = async () => {
    const result = await Axios.get("https://runar-juleliste.herokuapp.com/personer");
    setPersonListe(result.data);
  }

  const slettBruker = async (id) => {
      await Axios.post("https://runar-juleliste.herokuapp.com/deleteUser", {
          id: id,
        });
